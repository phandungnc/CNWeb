...
const StudentDashboard = () => {
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      setLoading(true);
      try {
        // Gọi API lấy khóa học của học sinh
        const res = await axios.get('/api/student/courses', {
           headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
        });
        const rawData = Array.isArray(res.data) ? res.data : [];
        setCourses(sortCourses(rawData)); // Sắp xếp luôn
      } catch (error) {
  ...
  // Hàm tính trạng thái để sắp xếp
      ...
      {loading ? <div style={{textAlign:'center'}}>Đang tải...</div> : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          {filteredCourses.length === 0 ? (
            <p style={{textAlign:'center', color:'#666'}}>Bạn chưa tham gia khóa học nào.</p>
          ) : (
